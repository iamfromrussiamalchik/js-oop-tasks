...
describe('es6', () => {
    ...
    describe('#Dictionary', () => {
        ...
        it('размер словаря изменяется корректно', () => {
            const dic = new core.Dictionary();
            dic.addWord("больница", "Krankenhaus")
            dic.addWord("самолёт", "Flugzeug")
            dic.addWord("бабочка", "Schmetterling")
            assert.strictEqual(dic.getSize(), 3)
            dic.deleteWord("самолёт")
        ...
        it('массив ключей генерируется корректно', () => {
            ...
            dic.addWord("самолёт", "Flugzeug")
            dic.addWord("бабочка", "Schmetterling")
            assert.deepStrictEqual(dic.getAllWords(), ["больница", "самолёт", "бабочка"])
        })
    });
});